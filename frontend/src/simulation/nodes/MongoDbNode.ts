...
export class MongoDbNode extends BaseNode {
  ...
  onTick(_simTimeMs: number, router: MessageRouter): void {
    ...
    const throughput = this.config.throughputPerSec ?? 35;
    ...
    for (let i = 0; i < toProcess; i += 1) {
      ...
      const dynamicFailureRate = Math.min(0.9, (this.config.failureRate ?? 0.015) + loadFactor * 0.08);
      ...
      const base = this.config.latencyMs ?? 35;
      const latency = Math.max(8, Math.round(base * (0.8 + Math.random() * 0.5)));
